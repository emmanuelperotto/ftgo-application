...
@Slf4j
public class VerifyConsumerReplier {
    private final SfnClient sfnClient;

    public void replyWithSuccess(VerifyConsumerCommand verifyConsumerCommand) {
        try {
            var reply = VerifyConsumerReply.buildFromCommand(verifyConsumerCommand);
            ...
            var request = SendTaskSuccessRequest.builder()
                    .taskToken(verifyConsumerCommand.taskToken())
                    .output(replyJSON)
                    .build();
            ...
            replyWithFailure(verifyConsumerCommand, e);
        }
    }

    public void replyWithFailure(VerifyConsumerCommand verifyConsumerCommand, Exception e) {
        try {
